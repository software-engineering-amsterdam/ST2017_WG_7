...
randomFlip x = do 
 ...
 -- k (maximum integer),n (length of list) random values
 -- 
genIntList :: IO [Int]
genIntList = do 
  k <- getRandomInt 20
...
-- {P} S {Q}
-- if P is true before S is executed,
...
prop_ordered (x:xs) = all (>= x) xs && prop_ordered xs
 ...
 -- k = counter
 -- n = number of tests
 -- r = special (in relation) property (pre or post)
...
-- fromIntegral = converts to num
-- invar, invar pre and post condition
...
-- parity is working here
-- whatever parity goes in must come out, True
-- specification is maintained

-- testInvar parity (succ.succ) [0..100]
-- x = parity, y = succ.succ
...
-- difference && .&&. types
-- infixl can use it between parameters
infixl 2 .&&. 
infixl 2 .||.
...
-- fog x = f(g(x)) function composition
